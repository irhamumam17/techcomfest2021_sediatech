	...
	render() {
		return(
			<React.Fragment>
				...
				{
					(this.context.loading.wallets) ?
						<Loading  size="large" status={this.context.loading.wallets} />
					:
						<table className="table-lg">
							...
							<tbody>
								{
									this.context.wallets.map((wallet , key) => {
										return(
											<tr key={key} >
												<td className="p-2 text-center">{key + 1}</td>
												<td>{ wallet.student }</td>
												<td>{ wallet.total }</td>
												<td>{ wallet.status }</td>
												<td className="col-badge">
													...
													<Tippy content="detail"  delay={300}>
														<Link to={`/admin/payment/wallet/${wallet.id}`} className="badge-icon bg-blue">
															<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
...
Wallet.contextType = PaymentContext;
