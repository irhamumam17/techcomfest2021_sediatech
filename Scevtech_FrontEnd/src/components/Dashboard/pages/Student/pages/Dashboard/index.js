...
class Dashboard extends Component {
	...
	componentDidMount() {
		// get data counts
		axios.get(`student/${this.context.id}/dashboard/count` , { headers: { Authorization: `Bearer ${this.context.token}` }, })
			.then(res => {
				this.setState({
					counts: res.data,
		...
		// get list theory	
		axios.get(`student/${this.context.id}/dashboard/theory` , { headers: { Authorization: `Bearer ${this.context.token}` }, })
			.then(res => {
				this.setState({
					theories: res.data,
		...
		// get list duty
		axios.get(`student/${this.context.id}/dashboard/duty` , { headers: { Authorization: `Bearer ${this.context.token}` }, })
			.then(res => {
				this.setState({
					duties: res.data,
		...
		// get data absent
		axios.get(`student/${this.context.id}/dashboard/absent` , { headers: { Authorization: `Bearer ${this.context.token}` }, })
			.then(res => {
				this.setState({
				...
				<motion.div className="dashboard-title"
					...
					<div>
						...
						<h4 className="text-md italic lowercase text-gray-400">
							...
							<Link to="/student/dashboard" className="hover:underline hover:text-blue-400">dashboard </Link> 
						</h4>
					</div>
				</motion.div>
				<div className="flex flex-row gap-4">
				{
					...
					:
						<React.Fragment>
							...
							<CardDashboard bg="bg-purple-400" icon="exam" delay=".6" value={this.state.counts.exam} name="Ujian" />
						</React.Fragment>
				}
				</div>
														...
														<tr key={key} >
															...
															<td className="col-badge" >
																<Tippy content="detail" delay={300}>
																	<Link to={`/student/theory/detail/${theory.id}`} className="badge-icon bg-indigo">
											...
											this.state.duties.map((duty , key) => {
												return(
													<tr key={key}>
														...
														<td className="col-badge" >
															<Tippy content="detail" delay={300}>
																<Link to={`/student/duty/detail/${duty.id}`} className="badge-icon bg-blue">
																	<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
																	  <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
																	</svg>
																</Link>
