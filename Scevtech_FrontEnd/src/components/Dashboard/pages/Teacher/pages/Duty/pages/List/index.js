				...
				<motion.div className="dashboard-title"
					...
					<div>
						...
						<h4 className="text-md italic lowercase text-gray-400">
							...
							<span className="mx-1">/</span>
							<Link to="/teacher/duty/list" className="hover:underline hover:text-blue-400">semua </Link> 
						</h4>
					</div>
				</motion.div>
				<motion.div
					...
					>
					<Link to="/teacher/duty/add" className="button-header" >Tambah Tugas</Link>
				</motion.div>
							...
							return(
								<div className="w-1/3" key={key} >
									<motion.div className="bg-white rounded shadow-md overflow-hidden"
										...
										>
										<img src={process.env.REACT_APP_URL + duty.thumbnail} alt="duty thumbnail" className="w-full h-56  object-cover mb-3" />
										<div className="px-4 pb-4">
											...
											<div className="flex flex-row justify-between items-center mt-2 pt-2 border-t-2 border-gray-200">
												<Link to={"/teacher/duty/detail/" + duty.id} className="text-white py-1 px-4 bg-blue-400 rounded inline-block mt-2 uppercase font-semibold text-sm transition duration-200 hover:bg-blue-500">Detail</Link>
												<p className="text-sm italic font-semilight text-red-500">{duty.deadline}</p>
											</div>
										</div>
									</motion.div>
								</div>
